...
const Prism: React.FC<PrismProps> = ({
  ...
  // Extract properties safely to avoid passing object references into the dependency array
  ...
  useEffect(() => {
    ...
    // Cast to any to bypass potential strict property checks in types if any
    (gl as any).disable((gl as any).DEPTH_TEST);
    (gl as any).disable((gl as any).CULL_FACE);
    ...
    const program = new Program(gl, {
      ...
      uniforms: {
        ...
        uPxScale: {
          value: 1 / (((gl as any).drawingBufferHeight || 1) * 0.1 * SCALE)
        },
        uTimeScale: { value: TS }
      }
    });
    ...
    const resize = () => {
      ...
      iResBuf[0] = (gl as any).drawingBufferWidth;
      iResBuf[1] = (gl as any).drawingBufferHeight;
      offsetPxBuf[0] = offsetX * dpr;
      offsetPxBuf[1] = offsetY * dpr;
      program.uniforms.uPxScale.value = 1 / (((gl as any).drawingBufferHeight || 1) * 0.1 * SCALE);
    };
